	namespace EnergyTrading.MDM.Test
{
    ...
    [TestFixture]
    public class when_a_search_for_a_location_with_two_results_is_paged_at_one_per_page : IntegrationTestBase
    {
        ...
        private static IList<Location> pageOne;
        private static Uri pageOneNextPage;
        private static IList<Location> pageTwo;
        ...
        {
            pageOneResponse = client.Post(ServiceUrl["Location"] + "search", content);
        ...
        protected static void Establish_context()
        {
			...
			Script.LocationData.CreateSearch(search, entity1, entity2);
        ...
        private static IList<Location> GetPeopleFromFeed(SyndicationFeed feed)
        {
            List<EnergyTrading.MDM.Contracts.Sample.Location> result =
                feed.Items.Select(syndicationItem => (XmlSyndicationContent)syndicationItem.Content).Select(
                    syndic => syndic.ReadContent<EnergyTrading.MDM.Contracts.Sample.Location>()).ToList();
