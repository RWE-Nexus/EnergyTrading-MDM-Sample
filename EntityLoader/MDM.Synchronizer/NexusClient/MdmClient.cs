namespace MDM.Loader.NexusClient
{
    ...
    public class MdmClient : IMdmClient
    {
        ...
        public WebResponse<TContract> Create<TContract>(TContract contract) where TContract : IMdmEntity
        {
            logger.DebugFormat("{0}: Create request sent", typeof(TContract).Name);
            return service.Create(contract);
        }

        public WebResponse<MdmId> CreateMapping<TContract>(
            int id, 
            string systemName, 
            ...
            bool defaultSystemMapping = false) where TContract : IMdmEntity
        {
            ...
            if (!result.IsValid)
            {
                ...
                logger.ErrorFormat(
                    "{0}: Unable to create mapping for {1} - {2}/{3}: Message: {4}", 
                    typeof(TContract).Name, 
                    id, 
                    systemName, 
                    mappingString, 
        ...
        public WebResponse<TContract> Get<TContract>(int id) where TContract : IMdmEntity
        {
            logger.DebugFormat("{0}: Looking for {1}", typeof(TContract).Name, id.ToString());
            return service.Get<TContract>(id);
        }

        public WebResponse<TContract> Get<TContract>(MdmId identifier) where TContract : IMdmEntity
        {
        ...
        {
            var result = service.GetMapping<TContract>(
                id, 
                x => x.SystemName == systemName && x.Identifier == mappingString);
            if (result.IsValid)
            {
        ...
        public WebResponse<IList<TContract>> Search<TContract>(Search search) where TContract : IMdmEntity
        {
