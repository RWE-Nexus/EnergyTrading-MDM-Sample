namespace MDM.Loader.NexusClient
{
    ...
    public interface IMdmClient
    {
        ...
        WebResponse<MdmId> CreateMapping<TContract>(int id, string systemName, string mappingString, bool defaultSystemMapping = false)
            where TContract : IMdmEntity;

        WebResponse<TContract> Update<TContract>(int id, TContract entity, string etag)
            where TContract : IMdmEntity;

        WebResponse<TContract> DeleteMapping<TContract>(int id, int mappingId)
            where TContract : IMdmEntity;
    }
}