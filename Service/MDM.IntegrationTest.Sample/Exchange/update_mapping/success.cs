namespace EnergyTrading.MDM.Test
{
    ...
    [TestFixture]
    public class when_a_request_is_made_to_update_a_exchange_mapping : IntegrationTestBase
    {
        ...
        protected static void Because_of()
        {
            ...
            response = client.Post(ServiceUrl["Exchange"] +  string.Format("{0}/Mapping/{1}", entity.Id, currentTrayportMapping.Id), content);
        }

        [Test]
        public void should_update_the_mapping_on_the_exchange_entity()
        {
            var savedMapping = new DbSetRepository(new DbContextProvider(() => new SampleMappingContext())).FindOne<MDM.Exchange>(entity.Id).Mappings[0];
