namespace EnergyTrading.MDM.Test
{
    ...
    [TestFixture]
    public class when_a_request_is_made_to_create_a_exchange_entity : IntegrationTestBase
    {
        private static HttpResponseMessage response;
        private static EnergyTrading.MDM.Contracts.Sample.Exchange exchange;
        ...
        {
            response = client.Post(ServiceUrl["Exchange"], content);
            response.AssertStatusCodeIs(HttpStatusCode.Created);
        }

        [Test]
        public void should_create_an_instance_of_the_exchange_in_the_database_with_the_correct_details()
        {
            ExchangeDataChecker.ConfirmEntitySaved(int.Parse(GetLocationHeader()[1]), exchange);
        }
