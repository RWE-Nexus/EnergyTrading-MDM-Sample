namespace EnergyTrading.MDM.Test
{
    ...
    [TestClass]
    public class when_a_request_is_made_for_a_partyrole_and_they_dont_exist : IntegrationTestBase
    {
        ...
        public void should_return_nexus_failure_with_correct_information()
        {
            ...
            Assert.AreEqual("Unknown PartyRole", fault.Reason);
            Assert.AreEqual("PartyRole identified by '" + int.MaxValue + "' not found", fault.Message);
    ...
    [TestClass]
    public class when_a_request_is_made_for_a_partyrole_as_of_a_date_and_they_dont_exist : IntegrationTestBase
    {
        private static HttpResponseMessage response;
        private static HttpClient client;
        ...
        protected static void Because_of()
        {
            client = new HttpClient(ServiceUrl["PartyRole"] + string.Format("{0}?as-of=2010-03-16T11:21:23Z", int.MaxValue));
            response = client.Get();
        }
        ...
        public void should_return_nexus_failure_with_correct_information()
        {
            ...
            Assert.IsNotNull(fault);
            Assert.AreEqual("Unknown PartyRole", fault.Reason);
            Assert.AreEqual("PartyRole identified by '" + int.MaxValue + "' not found at the given date '" + date + "'", fault.Message);
            Assert.AreEqual(int.MaxValue.ToString(), fault.Identifier);
