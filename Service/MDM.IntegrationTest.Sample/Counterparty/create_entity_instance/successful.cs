namespace EnergyTrading.MDM.Test
{
    ...
    [TestFixture]
    public class when_a_request_is_made_to_create_a_counterparty_entity : IntegrationTestBase
    {
        private static HttpResponseMessage response;
        private static EnergyTrading.MDM.Contracts.Sample.Counterparty counterparty;
        ...
        {
            response = client.Post(ServiceUrl["Counterparty"], content);
            response.AssertStatusCodeIs(HttpStatusCode.Created);
        }

        [Test]
        public void should_create_an_instance_of_the_counterparty_in_the_database_with_the_correct_details()
        {
            CounterpartyDataChecker.ConfirmEntitySaved(int.Parse(GetLocationHeader()[1]), counterparty);
        }
