namespace EnergyTrading.MDM.Data.Search
{
    ...
    {
        public ICollection<Party> Execute(
        ...
        private IQueryable<Party> UnlimitedDetailTemporalQuery(
            IRepository repository, 
            string query, 
            DateTime? at, 
            string orderBy)
        {
            ...
            var queryable =
                repository.Queryable<PartyDetails>()
                    .Include("Party")
            ...
            return queryable.Select(x => x.Party).Include("Mappings").Include(y => y.Details);
        }
    }
}